      ...
      {tab==="alumnos"&&(
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderTop:"none",borderRadius:"0 0 10px 10px"}}>
          ...
          {alumnos.map((a,i)=>{
            ...
            return(
              <div key={a.uid}>
                <div onClick={()=>setAlumnoSel(a)} style={{padding:"11px 16px",cursor:"pointer",transition:"background .15s"}} onMouseEnter={e=>e.currentTarget.style.background=C.cardHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                  <div style={{display:"flex",alignItems:"center",gap:12}}>
                    <div style={{position:"relative"}}>
                      <div style={{width:34,height:34,borderRadius:"50%",background:C.blue,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:13,color:C.white}}>{(a.nombre||"?")[0]}</div>
                      {cicloAlerta&&<div style={{position:"absolute",top:-2,right:-2,width:10,height:10,borderRadius:"50%",background:C.pink,border:`2px solid ${C.card}`}}/>}
                    </div>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontWeight:700,fontSize:13,color:C.white}}>{a.nombre}</span>{a.genero==="F"&&<span style={{fontSize:9,color:C.pink}}>♀</span>}</div>
                      <div style={{fontSize:9,color:C.muted}}>{a.tipo||"Sin plan"}</div>
                    </div>
            ...
            <div>
              ...
              <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"12px 14px",marginTop:12}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  <div><div style={{fontSize:8,color:C.muted,marginBottom:4}}>TIPO</div><select value={tipo} onChange={e=>setTipo(e.target.value)} style={inp}><option value="">Sin asignar</option><option value="Solo Running">Solo Running</option><option value="Running + Gym">Running + Gym</option></select></div>
                  ...
                  <div><div style={{fontSize:8,color:C.muted,marginBottom:4}}>PAGO</div><select value={pagado?"si":"no"} onChange={e=>setPagado(e.target.value==="si")} style={inp}><option value="si">AL DÍA</option><option value="no">PENDIENTE</option></select></div>
                </div>
...
function KmChartAdmin({data,onAgregar}){
  ...
  return(
    <div>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"12px 14px",marginTop:12}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <input value={nueva.semana} onChange={e=>setNueva({...nueva,semana:e.target.value})} placeholder="S1" style={{flex:1,padding:"8px 10px",background:C.card,border:`1px solid ${C.border}`,borderRadius:6,color:C.white,fontSize:11,fontFamily:"inherit",outline:"none"}}/>
  ...
  useEffect(()=>{
    const cargar=async()=>{
      ...
      if(snap.exists()){
        ...
        setPeso(data.peso||"");
        setCinco(data.marcas?.cinco||"");
        setDiez(data.marcas?.diez||"");
        setMedia(data.marcas?.media||"");
        setMaraton(data.marcas?.maraton||"");
      }
